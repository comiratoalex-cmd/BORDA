...
borderW:6,
mode:'themes',
colors:['#ffb3c6','#ff8fab','#fb6f92','#ffc2d1'],
hue:0
}
...
box.style.setProperty('--borderW',STATE.borderW+'px')
STATE.colors.forEach((c,i)=>{
box.style.setProperty('--c'+(i+1),c)
})
box.style.setProperty('--glow',STATE.colors[2])
}
...
qs('borderW').oninput=e=>{STATE.borderW=e.target.value;apply()}
qs('mode').onchange=e=>{
...
STATE.colors=base.map(c=>rotateHue(c,parseInt(e.target.value)))
apply()
}
...
qs('resultLink').textContent=link
